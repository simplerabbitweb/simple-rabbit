#!/usr/bin/env python3
"""
post.py — Manual blog post publisher for Simple Rabbit

Usage:
  python3 post.py "Your Post Title" "A short description of what this post covers"

Optional:
  python3 post.py "Title" "Description" --category "SEO & Visibility"

Categories available:
  SEO & Visibility | Web Design | Patient Growth | Practice Marketing | Local SEO
"""

import os, json, re, sys, subprocess, urllib.request, urllib.parse
...
def generate_content(title, description, category=None):
    """Call Claude API. Returns dict with html, h1, category, excerpt, meta_description, unsplash_query."""
    import anthropic
    ...
    cat_line = (
        f"Category: {category}"
        if category
        else "Choose the best-fit category from: SEO & Visibility, Web Design, Patient Growth, Practice Marketing, Local SEO."
    )
...
- "category": the article category label (use provided one or choose best fit)
...
- "unsplash_query": 2-4 word Unsplash search query for a professional featured image (clean workspace, wellness, laptop, woman professional, medical, etc.)
...
<div id="announcement-banner" style="background:#829AAB;padding:11px 24px;text-align:center;position:relative;">
  <a href="/playbook" style="font-size:13px;color:#fff;font-family:'DM Sans',system-ui,sans-serif;text-decoration:none;display:inline-flex;align-items:center;gap:8px;flex-wrap:wrap;justify-content:center;">
    <span style="font-weight:400;opacity:0.88;">Free 5-day email series &rarr;</span>
    <strong style="font-weight:600;letter-spacing:0.1px;">The Fully-Booked Private Practice Playbook</strong>
...
def build_html(title, generated, slug, category, img_credit, pub_date_display, pub_date_iso):
    h1        = generated.get("h1", title)
    meta      = generated.get("meta_description", title)
...
<section style="position:relative;min-height:500px;display:flex;align-items:center;justify-content:center;overflow:hidden;">
  <div style="position:absolute;inset:0;background-image:url('/CTA-banner.jpg');background-size:cover;background-position:center;background-attachment:fixed;"></div>
  <div style="position:relative;z-index:1;background:rgba(255,255,255,0.18);backdrop-filter:blur(24px) saturate(160%);-webkit-backdrop-filter:blur(24px) saturate(160%);border:1px solid rgba(255,255,255,0.28);box-shadow:0 8px 32px rgba(0,0,0,0.10);padding:64px 56px;max-width:520px;width:calc(100% - 48px);text-align:center;">
...
def deploy(slug, pub_date_display):
    msg = f"New post: {slug} ({pub_date_display})"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Publish a blog post to Simple Rabbit",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python3 post.py "Why Your About Page Is Losing You Patients" "Most practice about pages talk about credentials. The ones that convert talk about the patient."
  python3 post.py "Title" "Description" --category "SEO & Visibility"
        """
    )
    parser.add_argument("title",       help="Post title")
    parser.add_argument("description", help="Brief description of what this post covers and the angle to take")
    parser.add_argument("--category",  default=None, help="Article category (optional — Claude will choose if omitted)")
    args = parser.parse_args()

    slug = slugify(args.title)

    log("=" * 60)
    log(f"Publishing: {args.title}")
    log(f"Slug: {slug}")

    blog_path = os.path.join(BASE, "blog", f"{slug}.html")
    if os.path.exists(blog_path):
        log(f"ERROR: blog/{slug}.html already exists. Delete it first or choose a different title.")
        sys.exit(1)
    ...
    generated      = generate_content(args.title, args.description, args.category)
    h1             = generated.get("h1", args.title)
    category       = generated.get("category", args.category or "Web Design")
    ...
    html = build_html(args.title, generated, slug, category, img_credit, pub_date_display, pub_date_iso)
    with open(blog_path, "w") as f:
        f.write(html)
    log(f"  Saved blog/{slug}.html")

    # 4. Update articles.html
    update_articles_html(slug, h1, category, excerpt, pub_date_display)
